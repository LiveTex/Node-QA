...
qa.run = function(base, names, log, complete, opt_contextFactory) {
  ...
  if (names.length > 0) {
    ...
    for (var i = 0; i < list.length; i += 1) {
      ...
      if (test !== undefined) {
        suite.push(qa.__case(name, test, contextFactory));
      } else {
        var input = base[name + qa.APP_CASE_INPUT_SUFFIX] || async.nop;
        var app = base[qa.APP_CASE_PREFIX + name];
        if (app !== undefined) {
          suite.push(qa.__applicationCase(name, input, test, contextFactory));
        }
      }
    }
  } else {
    for (var key in base) {
      if (key.indexOf(qa.CASE_PREFIX) === 0) {
        ...
        suite.push(qa.__case(key.substr(qa.CASE_PREFIX.length),
            base[key], contextFactory));

      } else if (key.indexOf(qa.APP_CASE_PREFIX) === 0) {
        name = key.substr(qa.APP_CASE_PREFIX.length);

        suite.push(qa.__applicationCase(name,
 ...
 * @param {function(string):!async.Actor} contextFactory Имя.
 * @param {number=} opt_timeout Максимальное время выполнения.
 * @return {!async.Actor} Тест.
 */
qa.__case = function(name, actor, contextFactory, opt_timeout) {
  var timeout = opt_timeout || qa.CASE_TIMEOUT;

  return async.context.isolate(contextFactory(name),
 ...
 * @return {!async.Actor} Тест.
 */
qa.__applicationCase =
    function(name, input, scenarios, contextFactory, opt_timeout) {
  var timeout = opt_timeout || qa.APP_CASE_TIMEOUT;
  var test = [];

  for (var key in scenarios) {
    test.push(qa.__case(name + '/' + key, scenarios[key],
...
qa.__exceptionLogger = function(error, opt_code) {
  ...
  function pass(data, complete, cancel) {
    this.getEventLog().pass(new qa.test.events.Exception(
        this.getName(), true, 'Unhandled exception.', error));
