 ...
 * теста - утверждения, состояния, варианты проверки и их объединения.
 *
 * @namespace
 */
var qa = {};
...
/**
 * Описание варианта тестирования взаимодействия в базовом объеткте теста -
 ...
 * описания сценария.
 *
 * Имена объектов описания в базовом объекте теста начинаются с соответствующих
 * префиксов.
 *
 * @see qa.TestBase
 *
...
qa.__createContext = function(name) {
  ...
  /**
   * @this {qa.result.IFolderFactory}
   * @param {*} data Данные.
   * @param {function(!qa.result.IFolder)} complete Обработчик завершения.
   * @param {function(string, number=)} cancel Обрабтчик ошибки.
   */
  function factory(data, complete, cancel) {
    complete(this.createFolder(name));
  }
...
qa.__errorCatcher = function(message, opt_code) {
  ...
  function catcher(data, complete, cancel) {
    var result = this.getFolder().getResult();
...
qa.__passCurrentResult = function(tag) {
  ...
  function pass(data, complete, cancel) {
    this.getFolder().passCurrentResult(tag);
    complete(data);
  }
