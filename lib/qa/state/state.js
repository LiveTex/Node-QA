...
/**
 * Создание шага установки входных данных в качестве значения по
 * выбранному пути.
 *
 * @see async.Step
 *
...
/**
 * Создание шага добавления входных данных к значению по
 * выбранному пути.
 *
 * @see async.Step
 *
...
qa.state.append = function(path) {
   ...
   * @param {!async.ErrorHandler} cancel Обработчик ошибки.
   */
  function append(data, complete, cancel) {
...
/**
 *
 * Создание шага добавления строки к значению по
 * выбранному пути.
 *
 *  @see async.Step
 * @param {string} path Выбранный путь.
 ...
 * @return {!async.Step} Созданный шаг.
 */
qa.state.appendString = function(path, string) {
  var tokens = path.split(qa.state.PATH_SEPARATOR);
...
/**
 * Создание шага передачи данных в следующий шаг.
 *
 * @param {*} insertData
 * @return {!async.Step} Созданный шаг.
 */
qa.state.insertData = function(insertData) {

  function insert(data, complete, cancel) {
    complete(insertData);
  }
  return insert;
};
...
/**
 * Создание шага условного копирования. Проверяет путь назначения на
 * налчиие значения !null и копирует туда значение из из пути "откуда".
 *
 * @param {string} pathFrom Путь откуда
 * @param {string} pathTo Путь куда
 * @return {!async.Step} Созданный шаг.
 */
qa.state.copynx = function(pathFrom, pathTo) {

  return async.script.sequence([
    async.script.if(
        qa.state.condition.equalsTo(pathTo, null),
        qa.state.copy(pathFrom, pathTo)
    )
  ]);
 ...
 * @return {!async.Step} Созданный шаг.
 */
qa.state.mapInput = function(step) {
  var token = (Math.random() + (qa.state.__tempTokenIndex += 1));
