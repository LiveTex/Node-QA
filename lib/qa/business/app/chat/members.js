 ...
 */
qa.business.app.chat.auth = function(holder, complete, cancel) {
 ...
 * @param {function(string, number)} cancel Error handler.
 */
qa.business.app.chat.chatList = function(holder, complete, cancel) {
  var member = holder.getMember();
  this.getConnectionByUser(member).request(
 ...
 * @param {function(string, number)} cancel Error handler.
 */
qa.business.app.chat.visitorList = function(holder, complete, cancel) {
  var member = holder.getMember();
  this.getConnectionByUser(member).request(
      new qa.business.comm.ChatServerRequest('chat_list', {
        'site_id': 1
