...
/**
 * @this {*} Data.
 * @param {!qa.business.entity.IMemberHolder} holder Visitor.
 * @param {function(*)} complete
 *   Success handler.
 * @param {function(string, number=)} cancel Error handler.
 */
qa.business.app.member.offline = function(holder, complete, cancel) {
  this.disconnect();
  complete(holder);
};
...
qa.business.app.member.assertHasChatsCount = function(count) {
  return function(holder, complete, cancel) {
    var member = holder.getMember();
    if (Object.keys(member.getChatVisitors()).length === count) {
      complete(holder);
    } else {
