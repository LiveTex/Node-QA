 ...
 */
qa.report.addNode = function(node, path, obj) {
  var step = path.shift();
  if (path.length === 0) {
    obj[step] = node;
  } else if ((obj[step] !== null) && (typeof obj[step] === 'object')) {
    qa.report.addNode(node, path, obj[step]);
  }
};
  ...
  for (var i in items) {
    ...
    switch (item.getType()) {
      case qa.report.ReportItemType.TEST_CASE_STARTED:
        ...
        qa.report.addNode({}, path.slice(0), result);
        break;
      case qa.report.ReportItemType.TEST_CASE_STOPPED:
        path.pop();
        break;
      case qa.report.ReportItemType.ASSERTION_RESULT:
        qa.report.addNode(item.getValue(), path.slice(0).concat("Assertion#" + item.getId()), result);
        break;
    }
  }
  return result;
