...
/**
 * Подготавливает options для запроса с методом GET
 * @param {!String} path
 * @return {*}
 */
qa.ext.ui.Browser.prototype.__prepareGetSettings =
    function(path) {

  var self = this;

  return qa.ext.ui.Browser.prototype.__prepareSeleniumSettings(self,
      '/session/' + self.__sessionId + path, 'GET');
};


/**
 * Подготавливает options для запроса с методом POST
 * @param {!String} path
 * @return {*}
 */
qa.ext.ui.Browser.prototype.__preparePostSettings =
    function(path) {

  var self = this;
  return qa.ext.ui.Browser.prototype.__prepareSeleniumSettings(self,
      '/session/' + self.__sessionId + path, 'POST');
};


/**
 * Подготавливает options для запроса с методом DELETE
 * @param {!String} path
 * @return {*}
 */
qa.ext.ui.Browser.prototype.__prepareDeleteSettings =
    function(path) {

  var self = this;
  return qa.ext.ui.Browser.prototype.__prepareSeleniumSettings(self,
      '/session/' + self.__sessionId + path, 'DELETE');
};
...
qa.ext.ui.Browser.prototype.setTimeout = function(type, time) {
  ...
  function setTimeout(input, complete, cancel) {
    ...
    var seleniumSettings = self.__preparePostSettings('/timeouts');
...
qa.ext.ui.Browser.prototype.setUrl = function(url) {
  ...
  function setUrl(input, complete, cancel) {
    ...
    var seleniumSettings = self.__preparePostSettings('/url');
  ...
  function findElement(input, complete, cancel) {
    ...
    function localComplete(data, response) {
      ...
      if (decodedResponse !== null) {
        ...
        if (decodedResponse['status'] === 0) {
          self.__lastFindElement = decodedResponse['value'].ELEMENT;
          complete(self.__lastFindElement);
        } else {
    ...
    var seleniumSettings =
        self.__preparePostSettings('/element');
...
qa.ext.ui.Browser.prototype.produceElementAction = function(method, opt_value) {
  ...
  function produceElementAction(input, complete, cancel) {
    ...
    var seleniumSettings =
        self.__preparePostSettings('/element/' + input + '/' + method);
...
qa.ext.ui.Browser.prototype.getElementData = function(type, opt_attributeName) {
  ...
  function getElementData(input, complete, cancel) {
    ...
    if (type != 'attribute') {
      seleniumSettings =
          self.__prepareGetSettings('/element/' + input + '/' + type);
    } else {
      seleniumSettings =
          self.__prepareGetSettings('/element/' + input + '/' + type +
                  '/' + opt_attributeName);
